/**
 * Gonçalo Candeias Amaro 17440 - King
 */
...
public class King extends Pieces {
     ...
     */
    public King(Model model, Position position, Model.PieceColour color) {
        this.model = model;
        this.color = color;
    ...
    public boolean canMoveTo(int line, char colChar) {
        ...
        if (this.model.notEmpty(line, col) && this.model.isInside(line, col)) {
            for (int l = -1; l <= 1; l++) {
                for (int c = -1; c <= 1; c++) {
                    if (line == this.position.getLine() + l && col == this.position.colTranslate(this.position.getCol()) + c) {
                        if (!(l == 0 && c == 0)) {
                            return true;
                        }
                    }
    ...
    public ImageView getImage() {
        ...
        if (color == Model.PieceColour.WHITE) {
            return new ImageView(
                    new Image("resources/WhiteKing.png", 80, 80, false, false)
            );
        } else {
            return new ImageView(
                    new Image("resources/BlackKing.png", 80, 80, false, false)
            );
        }
