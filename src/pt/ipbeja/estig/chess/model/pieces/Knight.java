/**
 * Gonçalo Candeias Amaro 17440 - Knight
 */
...
public class Knight extends Pieces {
     ...
     */
    public Knight(Model model, Position position, Model.PieceColour color) {
        this.model = model;
        this.color = color;
    ...
    public boolean canMoveTo(int line, char colChar) {
        ...
        if (this.model.isInside(line, col) && this.model.notEmpty(line, col)) {
            if (line == this.position.getLine() - 2 &&
                    (col == this.position.colTranslate(this.position.getCol()) - 1
                            || col == this.position.colTranslate(this.position.getCol()) + 1)) return true;
            ...
            if (col == this.position.colTranslate(this.position.getCol()) - 2 &&
                    (line == this.position.getLine() - 1 || line == this.position.getLine() + 1)) return true;
    ...
    public ImageView getImage() {
        ...
        if (color == Model.PieceColour.WHITE) {
            return new ImageView(
                    new Image("resources/WhiteKnight.png", 80, 80, false, false)
            );
        } else {
            return new ImageView(
                    new Image("resources/BlackKnight.png", 80, 80, false, false)
            );
        }
    ...
    public boolean canKill(int line, char colChar) {
        ...
        if (this.model.isInside(line, col) && (this.model.getPiece(line, col) != this.model.empty)) {
            ...
            if (col == this.position.colTranslate(this.position.getCol()) - 2 &&
                    (line == this.position.getLine() - 1 || line == this.position.getLine() + 1)) {
                if (this.model.getPiece(line, col).getColor() != this.color) return true;
            }
